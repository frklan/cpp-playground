...
MainWindow::MainWindow() {
  ...
  Logger::info("MainWindow instantiating");
  ...
  auto logView = std::make_shared<LogView>(this->lines() - 3, this->cols() / 2, 3, 0);
  logView->label("logView");
  this->addView(logView);
  this->activateNextView();

  auto logView2 = std::make_shared<LogView>((this->lines() - 3) / 2, this->cols() / 2, 3, this->cols() / 2);
  logView2->label("logView2");
  this->addView(logView2);

  auto logView3 = std::make_shared<LogView>((this->lines() - 3) / 2, this->cols() / 2, this->lines() / 2 + 2 , this->cols() / 2);
...
int MainWindow::run() {
  Logger::info("MainWindow loop begin");
  ...
  do {
    ...
    switch(chr) {
      case KEY_RESIZE:
        Logger::info("KEY_RESIZE event detected");
        break;
      case KEY_TAB: 
        Logger::info("Switching active view");
        this->activateNextView();
        break;
      case KEY_F(1):
        Logger::info("F1 key pressed");
        break;
      case 'q':
      case KEY_ESC:
        Logger::info("KEY_ESC/q pressed, exiting!");
        quit = true;
        break;
      default:
        if(chr < KEY_MIN) {
          Logger::info("Passing keypress to active view (" 
              + std::get<0>(this->getActiveView()->getLabel()) + ")");
